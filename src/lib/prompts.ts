...
function autoGenerateDescription(prompt: Prompt, locale: string): string {
  ...
  const templates: Record<string, (title: string, text: string, tags: string) => string> = {
    en: (title, text, tags) =>
      `${title} prompt for Gempix2 running on the community "Nano Banana 2" stack. ${text} Keywords: ${tags}.`,
    zh: (title, text, tags) =>
      `${title} Prompt | 基于社区称作 "Nano Banana 2" 的 Gempix2 服务。${text} 关键词：${tags}。`,
    ja: (title, text, tags) =>
      `${title} プロンプト | コミュニティで「Nano Banana 2」と呼ばれる基盤上の Gempix2。${text} キーワード：${tags}。`,
    ko: (title, text, tags) =>
      `${title} 프롬프트 | 커뮤니티에서 'Nano Banana 2'라 부르는 스택으로 구동되는 Gempix2. ${text} 키워드: ${tags}.`,
  };
